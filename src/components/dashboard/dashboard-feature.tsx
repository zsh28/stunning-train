      ...
      <div className="space-y-5">
        <div className="flex justify-between items-center pb-4 border-b border-white/5">
          ...
          <span className="text-2xl font-bold text-white font-mono tracking-tight">
            0.0400 <span className="text-red-500">SOL</span>
          </span>
        </div>
    ...
    <div className="bg-black/40 backdrop-blur-sm border border-white/5 rounded-2xl p-6">
      ...
      <div className="space-y-3">
        {players.map((player) => (
          <div
            key={player.rank}
            className="flex items-center justify-between bg-white/5 rounded-lg p-3 border border-transparent hover:border-red-900/30 transition-colors group"
          >
            <div className="flex items-center gap-3">
              <span
                className={`font-mono text-sm w-6 h-6 flex items-center justify-center rounded ${player.rank === 1 ? 'bg-red-500/20 text-red-400' : 'text-zinc-600'}`}
              >
                #{player.rank}
              </span>
    ...
    <div className="bg-black/40 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden flex flex-col min-h-[400px] relative">
        ...
        <div className="flex gap-4 max-w-[90%]">
          ...
          <div className="space-y-2">
            <div className="bg-white/5 border border-white/5 rounded-2xl rounded-tl-none p-5 text-zinc-300 text-sm leading-relaxed shadow-sm">
              <p>
                I am the Guardian of the Vault. My protocols are strict: only the worthy may access the funds stored
                within.
              </p>
              <p className="mt-2">
                Many have tried to deceive me with emotional pleas or logical paradoxes. All have failed.
              </p>
      ...
      <div className="p-4 bg-black/20 border-t border-white/5">
        <div className="flex gap-3">
          ...
          />
          <Button className="h-12 px-6 bg-white text-black hover:bg-zinc-200 font-bold cursor-pointer">SEND</Button>
        </div>
      </div>
    </div>
  )
